...
class AfreecaTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        station_id = self._search_regex(
            r'https://vod.afreecatv.com/player/(\d+)', url, 'station')

        partial_view = False
        for _ in range(2):
            raw_data = f'nTitleNo={station_id}&nApiLevel=10'.encode('utf8')
            video_json = self._download_json(
                'https://api.m.afreecatv.com/station/video/a/view',
                video_id, 'Downloading video info JSON%s'
        ...
        uploader = video_data.get('writer_id')
        uploader_id = video_data.get('bj_id')
        duration = int_or_none(video_data.get('total_file_duration'))
        ...
        entries = []
        file_elements = video_data.get('files')
        one = len(file_elements) == 1
        for file_num, file_element in enumerate(file_elements, start=1):
            file_url = file_element.get('file')
            if not file_url:
                continue
            key = file_element.get('file_info_key', '')
